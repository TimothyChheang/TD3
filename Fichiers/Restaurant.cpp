...
void Restaurant::libererTable(int id, double montant, bool livraison) { 
	...
	for (unsigned i = 0; i < tables_.size(); ++i) {
		if (id == tables_[i]->getId()) {
			...
			chiffreAffaire_ -= calculerReduction(static_cast<Client*>(client),montant,livraison);
...
void Restaurant::commanderPlat(const string& nom, int idTable,TypePlat type, int nbIngredients) {
	...
	else
	{
		...
		else tables_[index]->commander(plat);
...
void Restaurant::livrerClient(Client * client, vector<string> commande)
{
	if (client->getStatut() == Prestige) {
		...
		for (unsigned i = 0; i < commande.size(); i++) {
			commanderPlat(commande[i],tables_[INDEX_TABLE_LIVRAISON]->getId(),client->getStatut(),tables_[INDEX_TABLE_LIVRAISON]->getCommande());

		}
		placerClients(client);
	}
}
...
double Restaurant::calculerReduction(Client* client, double montant, bool livraison) {
	...
	case Occasionnel:
		break;
	case Fidele:
		ClientRegulier* temp_clientReg;
		temp_clientReg = static_cast <ClientRegulier*> (client);
		if (temp_clientReg->getNbPoints() > SEUIL_DEBUT_REDUCTION)
			reduction = montant * TAUX_REDUC_REGULIER;
		break;
	case Prestige:
		ClientPrestige* temp_clientPres;
		temp_clientPres = static_cast <ClientPrestige*> (client);
		reduction = montant * TAUX_REDUC_PRESTIGE;
		if (temp_clientPres->getNbPoints() > SEUIL_LIVRAISON_GRATUITE)
			reduction += fraisTransport_[temp_clientPres->getAddresseCode()];
		break;
	}
}
